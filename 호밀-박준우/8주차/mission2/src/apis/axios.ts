...
// 전역 변수로 refresh 요청의 Promise를 지정해서 중복 요청을 방지한다.
...
axiosInstance.interceptors.request.use(
  (config) => {
    ...
    const accessToken = getItem(); //localStroage에서 accessToken을 가져온다.
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      // refresh 엔드포인트 401에러가 발생한 경우 (Unauthorized), 중복 재시도 방지를 위한 로그아웃 처리.
